...
public class Program {
    static int Main(string[] args) {
        ...
        var renderDot = new Option<bool>
            (name: "--render",
            description: "Render output dot files",
        ...
        genCfgTree.SetHandler(GenCfgTree, ueVersion, assetInputDirectory, cfgOutputDiretory, projectName, renderDot, dotPath);
...
Leading underscores can be used to work around special function names being illegal in the editor.");
    ...
    static void GenCfgTree(EngineVersion ueVersion, string assetInputDir, string output, string projectName, bool render, string? dotPath) {
        void RenderDot(string path) {
            ProcessStartInfo info = new ProcessStartInfo(dotPath ?? "dot");
            info.ArgumentList.Add(path);
            info.ArgumentList.Add("-Tsvg");
            info.ArgumentList.Add("-o");
            info.ArgumentList.Add(Path.ChangeExtension(path, ".svg"));
        ...
        Console.WriteLine("Building class hierarchy...");
        foreach (var assetPath in GetAssets(assetInputDir)) {
            ...
            if (classExport != null) {
                ...
                classNode.Attributes["URL"] = Path.Join("cfgs", Path.GetRelativePath(assetInputDir, Path.GetDirectoryName(assetPath)), Path.ChangeExtension(Path.GetFileName(assetPath), ".svg"));
        ...
        var hierarchyPath = Path.Join(output, "hierarchy.dot");
        var hierarchy = new StreamWriter(hierarchyPath);
        graph.Write(hierarchy);
        hierarchy.Close();
        if (render) {
            Console.WriteLine("Rendering class hierarchy...");
            RenderDot(hierarchyPath);
        }
